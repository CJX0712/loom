...
| `LOOM_CONSOLIDATE_MAX_TOKENS` | `8192` | 沉淀生成天花板（防失控）。**别调太小**：思维链与正文共用预算，实测 400/1024/3072 都会让正文为空 |
| `LOOM_CONSOLIDATE_TIMEOUT` | `1200` | 沉淀墙钟超时（秒）—— **这才是真正的边界**，超时就放弃本轮 |
...
- **记忆沉淀在 CPU + 思考型模型上要 10–20 分钟。** `qwen3:4b` 对"什么值得记"这类
  判断型任务，推理能超过 3000 token，按 4.5 tok/s 算光"想"就要十几分钟；**而且思维链
  与正文共用 `num_predict` 预算** —— 给小了（实测 400 / 1024 / 3072）模型把预算全用在
  "想"上，正文直接为空、一条都提不出来。代价可控是因为：跑在后台不阻塞对话、同时只允许
  一个（忙时跳过而非排队，连续聊天时这把频率天然压到每十几分钟一次）、可整体关闭。
